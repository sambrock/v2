...
import { projectSlideAnimateIn, projectSlideAnimateOut, projectSlideAnimateHoverIn, projectSlideAnimateHoverOut, projectSlideLinkAnimate } from '../animations/animations';
...
export default function ProjectSlide({ project, isActive, animate, current, indicator }) {
  ...
  return (
      <TransitionLink to={`/${project.title.toLowerCase()}`} exit={{ trigger: ({exit, node}) => projectSlideLinkAnimate(height, width).play(), length: 1}} entry={{delay: 1}} >
